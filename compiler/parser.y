...
ty_decl : TYPE ID args_opt '=' type { $$ = $5; } // TODO

args_opt :             { $$ = NULL; }
         | '(' ids ')' { $$ = $2; }

ids :            { $$ = node_vec(a, vec_new(a, sizeof(sid_t))); }
    | ids ',' ID { vec_add(a, (vec_p)&$1->as.vec, (any_t)&$2->as.id); $$ = $1; }
...
polys : ID           { $$ = node_vec(a, vec_sing(a, sizeof(sid_t), (any_t)&$1->as.id)); }
      | polys ',' ID { vec_add(a, (vec_p)&$1->as.vec, (any_t)&$3->as.id); $$ = $1; }
...
type : ID                        { $$ = $1; }
     ...
     | '<' ty_fields ';' ID '>'  { $$ = node_ty_variant(a, $2->as.vec); } // TODO
...
expr : ID | NUM | STR              { $$ = $1; }
     ...
     | expr '-' expr               { $$ = node_bop(a, $1, BOP_SUB, $3); }
     ...
     | ID ':' expr                 { $$ = $2; } // TODO
     | IF expr THEN expr ELSE expr { $$ = $2; } // TODO
...
arm : ID ID RARROW fn_body { $$ = node_arm(a, $1->as.id, $2->as.id, $4); }
